...
/* ----------------------------- MOVIMIENTOS (pestañas + paginación) ----------------------------- */
...
@page{size:A4;margin:6mm;} body{font-family:Arial;font-size:10px;color:#000;}
...
scanInput.addEventListener("input", async ()=>{
  ...
  try{
    ...
    if(tipoAccion==="entrada"){
      ...
      if(!movSnap.empty){
        let chosen=movSnap.docs[0]; let chosenTime=chosen.data().hora && chosen.data().hora.toDate ? chosen.data().hora.toDate() : new Date(0);
