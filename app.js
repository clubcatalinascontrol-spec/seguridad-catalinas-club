// app.js (parte 1/3) - Firebase 9.22
...
/* ----------------------------- Función para bloquear/desbloquear UI ----------------------------- */
...
// app.js (parte 2/3) - Usuarios y Novedades en tiempo real
...
onSnapshot(query(usuariosRef, orderBy("L","asc")), snapshot=>{
  ...
  snapshot.docs.forEach(d=>{
    ...
    tr.innerHTML=`<td>${u.L||""}</td><td>${(u.nombre||"").toUpperCase()}</td><td>${u.dni||""}</td><td>${u.celular||""}</td><td>${u.codigoIngreso||""}</td><td>${u.codigoSalida||""}</td><td>${u.tipo||""}</td>
      <td>
        <button class="ficha-btn" data-L="${u.L}">FICHA</button>
        <button class="edit-btn" data-id="${d.id}">EDITAR</button>
        <button class="del-btn" data-id="${d.id}">ELIMINAR</button>
      </td>`;
    ...
    tr.querySelector(".edit-btn").addEventListener("click", ()=>{
      ...
      document.getElementById("editUserModal").dataset.id = d.id;
...
/* ----------------------------- NOVEDADES en tiempo real con editar/eliminar ----------------------------- */
...
// app.js (parte 3/3) - Movimientos, Expirados con paginación, impresión y escaneo
...
function renderMovsPage(){
  ...
  page.forEach(item=>{
    const tr = document.createElement("tr");
    const aut = item.autorizante||"";
    tr.innerHTML = `<td>${item.L||""}</td><td>${(item.nombre||"").toUpperCase()}</td><td>${item.entrada||""}</td><td>${item.salida||""}</td><td>${item.tipo||""}</td><td class="autorizante-td">${aut}</td>
      <td>
...
function renderExpiredPage(pageNum){
  expiredTableBody.innerHTML="";
  const start = (pageNum-1)*MOV_LIMIT;
  const page = expiredCache.slice(start,start+MOV_LIMIT);
  page.forEach(e=>{
    const tr=document.createElement("tr");
    tr.innerHTML=`<td>${e.L||""}</td><td>${e.nombre||""}</td><td>${e.dni||""}</td><td>${e.codigoIngreso||""}</td><td>${e.codigoSalida||""}</td><td>${e.tipo||""}</td><td>${e.when ? fechaDDMMYYYY(e.when) : ""}</td>`;
  ...
  for(let p=1;p<=totalPages;p++){
    const btn=document.createElement("button");
    btn.textContent=p;
    if(p===pageNum){ btn.style.background="#d8a800"; btn.style.color="#111"; }
    btn.addEventListener("click", ()=>renderExpiredPage(p));
...
/* ----------------------------- ESCANEAR (permanece visible hasta cancelar) ----------------------------- */
...
function printMovimientosPorTipo(tipo){
  ...
  const toPrint = filtered.slice(0,25); // últimos 25
