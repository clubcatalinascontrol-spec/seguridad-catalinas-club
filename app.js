...
onSnapshot(query(usuariosRef, orderBy("L")), snapshot=>{
    ...
    snapshot.docs.forEach(docSnap=>{
        ...
        // listeners FICHA, EDITAR, ELIMINAR, IMPRIMIR se mantienen funcionales
        tr.querySelector(".ficha-btn").addEventListener("click", async ()=>{
            try{
                const snap2 = await getDocs(query(usuariosRef, where("L","==",u.L), limit(1)));
                if(!snap2.empty){
        ...
        tr.querySelector(".edit-btn").addEventListener("click", ()=>{ /* funcional */ });
        tr.querySelector(".del-btn").addEventListener("click", async ()=>{ /* funcional */ });
        tr.querySelector(".print-btn").addEventListener("click", async ()=>{ /* funcional */ });
    });
});
