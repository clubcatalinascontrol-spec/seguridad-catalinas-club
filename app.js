// app.js (módulo) - Firebase 9.22 (PARTE 1) - EDITADA
...
addUserBtn.addEventListener("click", async ()=>{
    ...
    if(dni && !/^\d{8}$/.test(dni)){ if(userMessage){ userMessage.style.color="red"; userMessage.textContent="DNI debe tener 8 dígitos"; setTimeout(()=>{ userMessage.textContent=""; userMessage.style.color=""; }, 2500);} return; }
    if(celular && !/^\d{10}$/.test(celular)){ if(userMessage){ userMessage.style.color="red"; userMessage.textContent="Celular debe tener 10 dígitos"; setTimeout(()=>{ userMessage.textContent=""; }, 2500);} return; }
    if(autorizante && !/^[A-Za-zÁÉÍÓÚáéíóúÑñ ]{1,12}$/.test(autorizante)){ if(userMessage){ userMessage.style.color="red"; userMessage.textContent="Autorizante inválido"; setTimeout(()=>{ userMessage.textContent=""; userMessage.style.color=""; }, 2500);} return; }
...
onSnapshot(query(usuariosRef, orderBy("L")), snapshot=>{
    ...
    snapshot.docs.forEach(docSnap=>{
        ...
        // (FICHA, EDITAR, ELIMINAR e IMPRIMIR listeners se mantienen iguales a tu versión)
    });
});
...
if (guardarNovedadBtn) {
    guardarNovedadBtn.addEventListener("click", async () => {
        ...
        if (!texto) { 
            novMsg.style.color = "red"; novMsg.textContent = "Ingrese texto"; 
        ...
        try {
            ...
            console.error(err);
            novMsg.style.color = "red"; novMsg.textContent = "Error";
            setTimeout(() => novMsg.textContent = "", 2000);
        }
    });
}
...
function renderMovsPage() {
    ...
    page.forEach(item => {
        ...
        const entradaTooltip = item.entrada ? item.entrada : "";
        const salidaTooltip = item.salida ? item.salida : "";
        tr.innerHTML = `
            <td>${item.L || ""}</td>
...
/* ----------------------------- ESCANEAR CÓDIGOS (mantiene modal abierto para múltiples) ----------------------------- */
...
scanInput.addEventListener("input", async () => {
    ...
    try {
        ...
        const nowStr = horaActualStr();
        if (tipoAccion === "entrada") {
            await addDoc(movimientosRef, { L: u.L, nombre: u.nombre, dni: u.dni || "", tipo: u.tipo, autorizante: u.autorizante || "", entrada: nowStr, salida: "", hora: serverTimestamp() });
        } else {
            const movQ = query(movimientosRef, where("L", "==", u.L), where("salida", "==", ""));
            const movSnap = await getDocs(movQ);
            if (!movSnap.empty) {
                let chosen = movSnap.docs[0];
                let chosenTime = chosen.data().hora?.toDate ? chosen.data().hora.toDate() : new Date(0);
                movSnap.docs.forEach(d => {
                    const t = d.data().hora?.toDate ? d.data().hora.toDate() : new Date(0);
                    if (t > chosenTime) { chosen = d; chosenTime = t; }
                });
                await updateDoc(doc(db, "movimientos", chosen.id), { salida: nowStr });
            } else {
                await addDoc(movimientosRef, { L: u.L, nombre: u.nombre, dni: u.dni || "", tipo: u.tipo, autorizante: u.autorizante || "", entrada: "", salida: nowStr, hora: serverTimestamp() });
            }
        }
...
function printMovimientosPorTipo(tipo, auto = false) {
    ...
    let html = `<html><head><title>${title}</title><style>
        @page{size:A4;margin:6mm;} body{font-family:Arial,Helvetica,sans-serif;font-size:10px;color:#000;} table{width:100%;border-collapse:collapse} th,td{border:1px solid #000;padding:2px;text-align:center;font-size:10px} thead th{background:#fff;font-weight:700;color:#000} img,svg{filter: grayscale(100%);}
