...
addUserBtn.addEventListener("click", async ()=>{
  ...
  try{
    ...
    if(userL) userL.value="NN"; if(userNombre) userNombre.value=""; if(userDni) userDni.value=""; if(userTipo) userTipo.value="NN";
...
onSnapshot(query(usuariosRef, orderBy("L")), snapshot=>{
  ...
  snapshot.docs.forEach(docSnap=>{
    ...
    tr.innerHTML=`${u.L||""} ${(u.nombre||"").toUpperCase()} ${u.dni||""} ${u.celular||""} ${u.autorizante||""} ${u.fechaExpedicion?fechaDDMMYYYY(u.fechaExpedicion):""} ${u.tipo||""} FICHA Editar Eliminar Imprimir Tarjeta`;
    usersTableBody.appendChild(tr);
    // Ficha, editar, eliminar, imprimir: se mantiene como en tu código original
  });
});
...
onSnapshot(query(movimientosRef, orderBy("hora","desc")), snapshot=>{
  const newMovs = snapshot.docs.map(d=>({__id:d.id,...d.data()}));
  // mantener todos los existentes y agregar los nuevos sin sobreescribir
  const existingIds = movimientosCache.map(m=>m.__id);
  newMovs.forEach(n=>{
    if(!existingIds.includes(n.__id)) movimientosCache.push(n);
  });
...
function renderExpiredPage(){
  ...
  page.forEach(item=>{
    const tr=document.createElement("tr");
    tr.innerHTML=`<td>${item.L||""}</td><td>${item.nombre||""}</td><td>${item.tipo||""}</td><td>${item.fechaExp?fechaDDMMYYYY(item.fechaExp):""}</td>`;
    expiredTableBody.appendChild(tr);
  });
  renderExpiredPagination(expiredCache.length);
}

onSnapshot(query(expiredRef, orderBy("fechaExp","desc")), snapshot=>{
