...
scanInput.addEventListener("input", async () => {
  ...
  const qEntrada = query(usuariosRef, where("codigoIngreso", "==", code));
  const snap = await getDocs(qEntrada);
  if(!snap.empty) { userDoc = snap.docs[0]; }
    ...
    else { scanMessage.style.color="red"; scanMessage.textContent="Código no válido"; return; }
  }
...
document.getElementById("restoreDefaultBtn").addEventListener("click", ()=>{
  if(confirm("Restaurar contraseña a 1234?")){
