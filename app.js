...
addUserBtn.addEventListener("click", async ()=>{
  ...
  if(!L || L==="NN" || !nombre || !tipo || tipo==="NN"){
    if(userMessage){ userMessage.style.color="red"; userMessage.textContent="Debe cargar nombre, #L y Tipo"; setTimeout(()=>{userMessage.textContent="";userMessage.style.color="";},3000);}
    return;
  }
  if(dni && !/^\d{8}$/.test(dni)){ if(userMessage){ userMessage.style.color="red"; userMessage.textContent="DNI 8 dígitos"; setTimeout(()=>{userMessage.textContent="";userMessage.style.color="";},2500);} return; }
  if(celular && !/^\d{10}$/.test(celular)){ if(userMessage){ userMessage.style.color="red"; userMessage.textContent="Celular 10 dígitos"; setTimeout(()=>{userMessage.textContent="";},2500);} return; }
  if(autorizante && !/^[A-Za-zÁÉÍÓÚáéíóúÑñ ]{1,12}$/.test(autorizante)){ if(userMessage){ userMessage.style.color="red"; userMessage.textContent="Autorizante max 12 letras"; setTimeout(()=>{userMessage.textContent="";userMessage.style.color="";},2500);} return; }
  nombre = nombre.toUpperCase();
  try{
    if(dni){
      const qDni=query(usuariosRef, where("dni","==",dni));
      const existing = await getDocs(qDni);
      if(!existing.empty){ if(userMessage){ userMessage.style.color="red"; userMessage.textContent="DNI ya registrado"; setTimeout(()=>{userMessage.textContent="";userMessage.style.color="";},2500);} return; }
    }
...
onSnapshot(query(usuariosRef, orderBy("L")), snapshot=>{
  ...
  snapshot.docs.forEach(docSnap=>{
    ...
    tr.innerHTML=`<td>${u.L||""}</td><td>${(u.nombre||"").toUpperCase()}</td><td>${u.dni||""}</td><td>${u.celular||""}</td><td>${u.autorizante||""}</td><td>${u.fechaExpedicion?fechaDDMMYYYY(u.fechaExpedicion):""}</td><td>${u.tipo||""}</td>
      <td><button class="ficha-btn">FICHA</button><button class="edit-btn">Editar</button><button class="del-btn">Eliminar</button><button class="print-btn">Imprimir</button></td>`;
    usersTableBody.appendChild(tr);

    // FICHA
    tr.querySelector(".ficha-btn").addEventListener("click", async ()=>{
      try{
        const snap = await getDocs(query(usuariosRef, where("L","==",u.L), limit(1)));
        if(!snap.empty){
...
/* ----------------------------- PARTE 2 - MOVIMIENTOS, EXPIRADOS, NOVEDADES ----------------------------- */
...
function renderPagination(totalItems, paginationElement){
...
let expiredCache=[], expiredPage=1, expiredFilter="todos";

function renderExpiredPage(){
  if(!expiredTableBody) return;
  expiredTableBody.innerHTML="";
  const filtered = expiredFilter==="todos"?expiredCache:expiredCache.filter(m=>m.tipo===expiredFilter);
  const start=(expiredPage-1)*MOV_LIMIT;
  const page=filtered.slice(start,start+MOV_LIMIT);
  page.forEach(item=>{
    const tr=document.createElement("tr");
    tr.innerHTML=`<td>${item.L||""}</td><td>${item.nombre||""}</td><td>${item.tipo||""}</td><td>${item.fecha?fechaDDMMYYYY(item.fecha):""}</td>`;
    expiredTableBody.appendChild(tr);
  });
  renderPagination(filtered.length, expiredPaginationDiv);
}

// filtros expirados
document.querySelectorAll(".expired-filter-btn").forEach(btn=>{
  btn.addEventListener("click", ()=>{
    document.querySelectorAll(".expired-filter-btn").forEach(b=>b.classList.remove("active"));
    btn.classList.add("active");
    expiredFilter=btn.dataset.tipo;
    expiredPage=1;
    renderExpiredPage();
  });
});
...
onSnapshot(query(expiredRef, orderBy("fecha","desc")), snapshot=>{
  ...
  const totalPages=Math.max(1,Math.ceil(expiredCache.length/MOV_LIMIT));
  if(expiredPage>totalPages) expiredPage=totalPages;
  renderExpiredPage();
});

/* ----------------------------- ESCANEO - siempre nueva fila ----------------------------- */
...
scanBtn.addEventListener("click", ()=>{
  ...
  scanInput.value=""; scanMessage.textContent=""; scanInput.focus();
});
...
scanInput.addEventListener("input", async ()=>{
  ...
  try{
    let userDoc=null; let tipoAccion="entrada";
...
if(novedadesTableBody){
  onSnapshot(query(novedadesRef, orderBy("when","desc")), snapshot=>{
    ...
    snapshot.docs.forEach(d=>{
      ...
      const tr=document.createElement("tr");
      let horaFecha="";
      if(n.when){ const date=parseToDate(n.when); if(date){ const hora=date.toLocaleTimeString("es-AR",{hour:"2-digit",minute:"2-digit"}); const fecha=date.toLocaleDateString("es-AR"); horaFecha=`${hora}<br><small>${fecha}</small>`; } }
      tr.innerHTML=`<td style="white-space:nowrap">${horaFecha}</td><td style="text-align:left; padding-left:8px;">${n.texto||""}</td>
      <td><button class="edit-nov" data-id="${d.id}">Editar</button><button class="del-nov" data-id="${d.id}">Eliminar</button></td>`;
      novedadesTableBody.appendChild(tr);

      tr.querySelector(".edit-nov").addEventListener("click", ()=>{
        novTxt.value=n.texto||""; editingNovedadId=d.id; document.querySelector('#novedades').scrollIntoView({behavior:'smooth'});
      });
