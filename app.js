...
const novedadTextoInput=document.getElementById("novedadTexto");
...
onSnapshot(query(novedadesRef, orderBy("when","desc")), snapshot=>{
  ...
  snapshot.docs.forEach(d=>{
    ...
    tr.querySelector(".edit-nov").addEventListener("click", ()=>{
      novedadTextoInput.value=n.texto||"";
...
/* Guardar novedad (editar o nueva) */
guardarNovedadBtn.addEventListener("click", async ()=>{
  if(!isUnlocked){ alert("Operación no permitida. Introduzca la contraseña de apertura."); return; }
  const texto = novedadTextoInput.value.trim();
  if(!texto) return;
  try{
    if(editingNovedadId){
      await updateDoc(doc(db,"novedades",editingNovedadId), { texto });
      editingNovedadId=null;
    } else {
      await addDoc(novedadesRef,{ texto, when: serverTimestamp() });
    }
    novedadTextoInput.value="";
...
function renderExpiredPage(){
  ...
  page.forEach(item=>{
    const tr=document.createElement("tr");
    let fechaStr=item.when ? (item.when.toDate ? item.when.toDate() : new Date(item.when)).toLocaleString("es-AR") : "";
    tr.innerHTML=`<td>${item.L||""}</td><td>${item.codigo||""}</td><td>${fechaStr}</td>`;
...
/* columna autorizante según tipo */
...
scanInput.addEventListener("input", async ()=>{
  ...
  try{
    ...
    scanInput.value=""; // mantener modal abierto para siguiente código
...
/* ----------------------------- IMPRIMIR MOVIMIENTOS CON ESTILO ESPECÍFICO ----------------------------- */
